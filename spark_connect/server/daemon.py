...
from spark_connect.config.configer import SparkEnvConfiger
from spark_connect.exceptions import DaemonError, UnconfigurableError
from spark_connect.log import logger
from spark_connect.server.tailer import Tailer
from spark_connect.utils import (
...
class Daemon:
    ...
    ENV_MASTER_MODE = "SPARK_CONNECT_SERVER_MODE"
    DEFAULT_MODE = "local"
    SPARK_IDENT_STRING = "spark_connect-server"  # SPARK_IDENT_STRING
    ...
    def __init__(
        ...
        *,
        ...
        if not exist_connect_server_package() and (SPARK_VERSION and SCALA_VERSION):
            # Specify the spark_connect package if no such package in $SPARK_HOME/jars
            # Spark will download the package from maven repo
            self.daemon_package.append(
                f"org.apache.spark:spark_connect_{SCALA_VERSION}:{SPARK_VERSION}"
            )
